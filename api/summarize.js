// /api/summarize.js — news/podcast summaries over the shared provider cascade.
// The six-provider chain now lives in api/_providers.js; this route just picks
// the 'fast' job (Groq-70b → Groq-8b → Gemini → Claude) and keeps its 24h edge
// cache. Modes: summary | takeaways | explain | briefing | briefing-gen.
import { requireAuth } from './_lib.js';
import { callAI, configuredProviders } from './_providers.js';
...
export default async function handler(req, res) {
  ...
  const r = await callAI({ job: 'fast', system, messages: [{ role: 'user', content: user }], maxTokens });

  if (r && r.text) {
    res.setHeader('Cache-Control', 's-maxage=86400, stale-while-revalidate=604800');
    return res.status(200).json({ summary: r.text, provider: r.provider, model: r.model });
  }
