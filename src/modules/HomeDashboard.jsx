...
import { getFeed, fmtCount, PLATFORM_META, PLATFORM_STATUS, TIER_META } from '../lib/adapters.js';
  ...
  return (
    <div style={{ marginBottom: isMobile ? 20 : 24, marginLeft: isMobile ? -14 : 0, marginRight: isMobile ? -14 : 0 }}>
      <div style={{ padding: isMobile ? '0 14px' : '0' }}>
        <SectionLabel icon="📡" label="Live Signal Feed" action={load} actionLabel="Refresh" />
      </div>
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 10, padding: isMobile ? '0 14px' : 0 }}>
          {feed.map(s => {
            ...
            const accent = meta.color || 'var(--accent, #00C6E6)';
            return (
