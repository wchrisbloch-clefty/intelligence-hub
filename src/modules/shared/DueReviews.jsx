  ...
  return (
    <div style={{ background: 'var(--surface)', border: `1px solid ${ACCENT}`, borderRadius: 14, padding: '16px 18px', marginBottom: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--dim)', marginLeft: 'auto' }}>{due.length} {due.length === 1 ? 'topic' : 'topics'}</div>
      </div>
        ...
        <div key={r.topicId} style={{ padding: '10px 0', borderTop: '1px solid var(--bord2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 'var(--fs-base)', fontWeight: 600, color: 'var(--text-b)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.topicLabel}</div>
              <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--dim)', marginTop: 2 }}>Last: {r.lastResult} · ease {r.ease}</div>
            </div>
            {onQuiz && (
              <div onClick={() => onQuiz(r.topicLabel)} title="Quiz me" style={{ flexShrink: 0, fontSize: 'var(--fs-sm)', color: ACCENT, cursor: 'pointer', fontWeight: 700 }}>Quiz →</div>
            )}
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            {GRADES.map(g => (
              <div key={g.id} onClick={(e) => grade(r.topicId, g.quality, e)}
                style={{ flex: 1, textAlign: 'center', padding: '6px 4px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--muted)', fontSize: 'var(--fs-sm)', fontWeight: 600, cursor: 'pointer' }}
