  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--bord2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--text)', fontFamily: "'Newsreader', serif" }}>Saved Sessions</div>
        <div onClick={onClose} style={{ fontSize: 13, color: 'var(--subtle)', cursor: 'pointer', padding: '2px 6px' }}>✕</div>
      </div>

      <div style={{ padding: '10px 12px', flexShrink: 0 }}>
        <div onClick={() => { onNew(); onClose(); }}
          style={{ padding: '10px 12px', borderRadius: 9, border: `1px solid ${color}`, color, background: 'var(--accent-glow)', cursor: 'pointer', fontSize: 12, fontWeight: 700, textAlign: 'center' }}>
          ＋ New session
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px 14px' }}>
        {items.length === 0 && (
          <div style={{ textAlign: 'center', padding: '30px 12px', color: 'var(--dim)', fontSize: 11, lineHeight: 1.7 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {editingId === s.id ? (
                  <input autoFocus value={editValue} onClick={e => e.stopPropagation()}
                    ...
                    style={{ width: '100%', background: 'var(--bg)', border: `1px solid ${color}`, borderRadius: 6, padding: '4px 7px', fontSize: 12, color: 'var(--text)', fontFamily: 'inherit', outline: 'none' }} />
                ) : (
                  <>
                    <div style={{ fontSize: 12, fontWeight: 600, color: active ? color : 'var(--text-b)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.title}</div>
                    <div style={{ fontSize: 9, color: 'var(--dim)', marginTop: 2 }}>{timeAgo(s.updatedAt)}</div>
                  </>
                )}
              </div>
              {editingId !== s.id && (
                <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                  <span onClick={e => startRename(s, e)} title="Rename" style={{ fontSize: 11, color: 'var(--subtle)', cursor: 'pointer', padding: '2px 4px' }}>✎</span>
                  <span onClick={e => del(s.id, e)} title="Delete" style={{ fontSize: 11, color: 'var(--red)', cursor: 'pointer', padding: '2px 4px' }}>🗑</span>
                </div>
              )}
            </div>
          );
